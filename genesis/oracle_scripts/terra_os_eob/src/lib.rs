...
fn prepare_impl(input: Input) {
    for i in 0..input.exchanges.len() {
        oei::ask_external_data(
            ...
            format!(
                "{} {} {}",
                input.exchanges[i], input.base_symbol, input.quote_symbol
            )
            .as_bytes(),
        );
    }
...
fn ds_output_to_ask_bid(arr: Vec<String>, multiplier: u64) -> OrderBook {
...
fn execute_impl(input: Input) -> Output {
    return Output {
        order_books: input
            ...
            .iter()
            .enumerate()
            .map(|(i, _)| ds_output_to_ask_bid(ext::load_input(i as i64), input.multiplier))
            .collect(),
    };
}
...
mod tests {
    ...
    fn test_ds_output_to_ask_bid() {
        ...
        assert_eq!(
            ...
            ds_output_to_ask_bid(vec!["12.33,11.65".into()], 100)
        );
        assert_eq!(
            OrderBook {
                ask: 5457,
            ...
            ds_output_to_ask_bid(vec!["29.01,30.24".into(), "54.57,17.27".into()], 100)
        );
        assert_eq!(
            OrderBook {
                ask: 4903,
        ...
        assert_eq!(
            ...
            ds_output_to_ask_bid(vec!["12.33,11.65".into(), "error1".into()], 100)
        );
        assert_eq!(
            OrderBook {
                ask: 9571,
        ...
        assert_eq!(
            ...
            ds_output_to_ask_bid(vec!["error1".into()], 100)
        );
        assert_eq!(
            OrderBook {
                ask: -1,
            ...
            ds_output_to_ask_bid(vec!["error1".into(), "error2".into()], 100)
        );
    }
}
