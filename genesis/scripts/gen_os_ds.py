...
def gen_data_source_oracle_script(owner, path):
...
def gen_oracle_script(file, os_source_dir, oracle_scripts, owner):
    for oracle_script in oracle_scripts:
        ...
        url = get_url(os.path.join(os_source_dir, pakage, "src", "lib.rs"), pakage)
...
def main():
    ...
    if len(sys.argv) != 2:
        print("Require path to data source and oracle script")
        return
    gen_data_source_oracle_script(owner, sys.argv[1])
